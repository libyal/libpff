/*
 * Library table_index_value type testing program
 *
 ...
 */
int pff_test_table_index_value_initialize(
     void )
{
	...
	/* Test regular cases
	 */
	result = libpff_table_index_value_initialize(
	          &table_index_value,
	          &error );
	...
	result = libpff_table_index_value_free(
	          &table_index_value,
	          &error );
	...
	table_index_value = (libpff_table_index_value_t *) 0x12345678UL;
	...
	for( test_number = 0;
	     ...
	     test_number++ )
	{
		/* Test libpff_table_index_value_initialize with malloc failing
		 */
	...
	for( test_number = 0;
	     ...
	     test_number++ )
	{
		/* Test libpff_table_index_value_initialize with memset failing
		 */
...
/* Tests the libpff_table_index_value_free function
 * Returns 1 if successful or 0 if not
 */
int pff_test_table_index_value_free(
     void )
{
	libcerror_error_t *error = NULL;
	int result               = 0;
...
#endif
{
	...
	PFF_TEST_RUN(
	 ...
	 pff_test_table_index_value_initialize );
