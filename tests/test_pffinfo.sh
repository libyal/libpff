...
TEST_EXECUTABLE="../pfftools/pffinfo";

if ! test -x "${TEST_EXECUTABLE}";
then
	TEST_EXECUTABLE="../pfftools/pffinfo.exe";
fi
...
TEST_PROFILE_DIRECTORY=$(get_test_profile_directory "input" "pffinfo");
...
";
	...
	then
		...
		do
			run_test_on_input_file_with_options "${TEST_SET_DIRECTORY}" "pffinfo" "with_stdout_reference" "${OPTION_SETS}" "${TEST_EXECUTABLE}" "${INPUT_FILE}" "${OPTIONS[@]}";
			RESULT=$?;
