...
/* TODO: this value is currently assumed based on the 512 x 8 = 4k page */
	...
	else
	{
		     ...
		     entry_index++ )
		{
			...
			result = libpff_recover_index_value_read_data(
			          index_value,
			          io_handle,
			          index_node->type,
			          node_entry_data,
			...
			if( result == -1 )
			{
				libcerror_error_set(
				 ...
				 LIBCERROR_ERROR_DOMAIN_IO,
				 LIBCERROR_IO_ERROR_READ_FAILED,
				 "%s: unable to read %s index entry: %" PRIu8 " value.",
				 function,
				 index_string,
				 entry_index );
...
int libpff_recover_index_value_read_data(
     ...
     libcerror_error_t **error )
{
	static char *function    = "libpff_recover_index_value_read_data";
	size_t value_size        = 0;
	...
	if( data_size < value_size )
	{
		...
		{
			libcnotify_printf(
			 "%s: %s index value data too small.\n",
			 function,
			 index_string );
		}
#endif
	...
	if( index_value->identifier == 0 )
	{
		...
		{
			libcnotify_printf(
			 "%s: %s index value has an empty identifier.\n",
			 function,
			 index_string );
		}
#endif
	...
	if( index_node_type == LIBPFF_INDEX_TYPE_DESCRIPTOR )
	{
		...
		if( index_value->data_identifier == 0 )
		{
			...
			{
				libcnotify_printf(
				 "%s: %s index value identifier: %" PRIu64 " has an empty data identifier.\n",
				 function,
				 index_string,
				 index_value->identifier );
			}
	...
	else if( index_node_type == LIBPFF_INDEX_TYPE_OFFSET )
	{
		...
		if( file_offset > (uint64_t) INT64_MAX )
		{
			...
			{
				libcnotify_printf(
				 "%s: %s index identifier: %" PRIu64 " has an invalid file offset: %" PRIi64 " (0x%" PRIx64 ").\n",
				 function,
				 index_string,
				 index_value->identifier,
				 (int64_t) file_offset,
		 ...
		 || ( (uint32_t) index_value->data_size > maximum_data_block_data_size ) )
		{
			...
			{
				libcnotify_printf(
				 "%s: %s index identifier: %" PRIu64 " has an invalid data size: %" PRIu16 ".\n",
				 function,
				 index_string,
				 index_value->identifier,
				 index_value->data_size );
