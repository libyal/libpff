...
void (*pfftools_signal_signal_handler)( pfftools_signal_t ) = NULL;
             ...
             pfftools_signal_t signal )
{
	...
	switch( signal )
	{
		...
		case CTRL_C_EVENT:
			if( pfftools_signal_signal_handler != NULL )
			{
				pfftools_signal_signal_handler(
				 signal );
			}
			return( TRUE );
...
{
	static char *function = "pfftools_signal_attach";
...
#if defined( _MSC_VER )
	pfftools_signal_initialize_memory_debug();
#endif
     ...
     libcerror_error_t **error )
{
	...
	pfftools_signal_signal_handler = NULL;
